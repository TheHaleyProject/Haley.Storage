...
namespace Haley.Utils {
    public partial class MariaDBIndexing : IDSSIndexing {
        public async Task<IFeedback> UpdateDocumentInfo(string moduleCuid, IOSSFileRoute file, string savedName) {
            ...
            try {
                ...
                var docvExists = await _agw.Scalar(new AdapterArgs(moduleCuid) { Query = INSTANCE.DOCVERSION.EXISTS_BY_ID }, (ID, file.Id));
                ...
                await _agw.NonQuery(new AdapterArgs(moduleCuid) { Query = INSTANCE.DOCVERSION.INSERT_INFO }, (ID, file.Id), (SAVENAME, savedName), (PATH, file.Path), (SIZE, file.Size));
                var updatedInfo = await _agw.Read(new AdapterArgs(moduleCuid) { Query = INSTANCE.DOCVERSION.GET_INFO, Filter = ResultFilter.FirstDictionary }, (ID, file.Id));
